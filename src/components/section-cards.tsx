...
import MetricsWrapper from '@/features/analytics/components/metrics-loading-wrapper';

export function SectionCards() {
  const { data: metricsData, isLoading } = useFetchMetrics();
  return (
    <MetricsWrapper isLoading={isLoading}>
      <div className='*:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4'>
        <Card className='@container/card'>
          <CardHeader>
            ...
            <CardAction>
              {Number(metricsData?.revenue?.growthRate || 0) > 0 ? (
        ...
        <Card className='@container/card'>
          <CardHeader>
            ...
            <CardAction>
              {Number(metricsData?.numOfOrders?.growthRate || 0) > 0 ? (
                <Badge variant='outline'>
                  <IconTrendingUp />+
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            ...
            <div className='text-muted-foreground'>
              last month was{' '}
              {formatCurrency(metricsData?.numOfOrders.lastMonth)}
            </div>
          </CardFooter>
        </Card>
        <Card className='@container/card'>
        ...
        <Card className='@container/card'>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            {Number(metricsData?.topSellerSaleCount?.growthRate || 0) > 0 ? (
              <div className='line-clamp-1 flex gap-2 font-medium'>
                Trending up this month <IconTrendingUp className='size-4' />
              </div>
            ) : Number(metricsData?.topSellerSaleCount?.growthRate || 0) > 0 ? (
